    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-500">Ringkasan</th>
            ...
            <tbody>
              {data.map((a) => (
                <tr key={a.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500 max-w-xs truncate">{a.body}</td>
                  <td className="px-4 py-3">
                    {a.pinned ? (
                      <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-100 text-yellow-700">Pinned</span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-600">Normal</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <button onClick={() => setDeleteId(a.id)} className="text-xs text-red-600 hover:underline">Hapus</button>
                  </td>
                </tr>
              ))}
              {data.length === 0 && (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Pengumuman' : 'Tambah Pengumuman'}>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Isi * (Markdown)</label>
            <div className="grid grid-cols-2 gap-3">
              <textarea value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} rows={8} className="w-full px-3 py-2 border rounded-lg font-mono text-sm" placeholder="**Bold**, *italic*, - list items..." />
              <div className="border rounded-lg p-3 overflow-auto max-h-64 bg-gray-50">
                <p className="text-xs text-gray-400 mb-2">Preview:</p>
                <div className="prose prose-sm max-w-none"><Markdown>{form.body || '_Tidak ada konten_'}</Markdown></div>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.pinned} onChange={(e) => setForm({ ...form, pinned: Number(e.target.value) })} className="w-full px-3 py-2 border rounded-lg">
            ...
            <div>
              ...
              <input type="date" value={form.expires_at} onChange={(e) => setForm({ ...form, expires_at: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
            </div>
          </div>
      ...
      <Modal open={deleteId !== null} onClose={() => setDeleteId(null)} title="Hapus Pengumuman">
        <p className="text-sm text-gray-600 mb-4">Yakin ingin menghapus pengumuman ini?</p>

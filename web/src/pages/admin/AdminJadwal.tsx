      ...
      {selected && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          {selected.schedule.length === 0 ? (
            <div className="p-8 text-center text-gray-400">Belum ada jadwal untuk kelas ini</div>
          ) : (
            selected.schedule.map((daySchedule) => (
              <div key={daySchedule.day} className="border-b last:border-0">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-gray-50/50">
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Jam</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">MK</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Tipe</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Dosen</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Ruang</th>
                      ...
                      {daySchedule.sessions.map((s, i) => (
                        <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <div className="text-xs text-gray-500">{s.course_name}</div>
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100">{s.type}</span>
                          </td>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.day} onChange={(e) => setForm({ ...form, day: e.target.value })} className="w-full px-3 py-2 border rounded-lg">
                {DAYS.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Jam</label>
              <input value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} className="w-full px-3 py-2 border rounded-lg" placeholder="08:00-09:30" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Kode MK</label>
              <input value={form.course_code} onChange={(e) => setForm({ ...form, course_code: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
            </div>
            <div>
          ...
          <div>
            ...
            <input value={form.course_name} onChange={(e) => setForm({ ...form, course_name: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Kode Dosen</label>
              <input value={form.lecturer_code} onChange={(e) => setForm({ ...form, lecturer_code: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Dosen</label>
              <input value={form.lecturer} onChange={(e) => setForm({ ...form, lecturer: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
            </div>
          </div>
          <div>
